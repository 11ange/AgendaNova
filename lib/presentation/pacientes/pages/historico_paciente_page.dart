...
class _HistoricoPacientePageState extends State<HistoricoPacientePage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<HistoricoPacienteViewModel>(context, listen: false)
        ...
        builder: (context, viewModel, child) {
          return Scaffold(
            appBar: CustomAppBar(
              ...
              onBackButtonPressed: () => context.pop(), // Volta para a tela anterior
            ),
                        ...
                        child: Column(
                          ...
                          children: [
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text('Responsável: ${viewModel.paciente!.nomeResponsavel}'),
                                    Text('Idade: ${viewModel.paciente!.idade} anos'),
                                    Text('Telefone: ${viewModel.paciente!.telefoneResponsavel ?? 'N/A'}'),
                                    Text('Email: ${viewModel.paciente!.emailResponsavel ?? 'N/A'}'),
                                    Text('Status: ${viewModel.paciente!.status == 'ativo' ? 'Ativo' : 'Inativo'}'),
                                    if (viewModel.paciente!.observacoes != null && viewModel.paciente!.observacoes!.isNotEmpty)
                                      Text('Observações: ${viewModel.paciente!.observacoes}'),
                                  ],
                                ),
                              ),
                            ),
                            ...
                            if (viewModel.treinamentos.isEmpty)
                              const Text('Nenhum treinamento encontrado para este paciente.')
                            else
                              ...viewModel.treinamentos.map((treinamento) {
                                return Card(
                                  ...
                                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                                  child: ExpansionTile(
                                    title: Text('Treinamento: ${treinamento.id ?? 'N/A'}'), // Substituir pelo nome do treinamento
                                    subtitle: Text('Início: ${treinamento.dataInicio} - Fim: ${treinamento.dataFim}'), // Exemplo
                                    children: [
                                      Padding(
                                        padding: const EdgeInsets.all(16.0),
                                        child: Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          children: [
                                            Text('Sessões:', style: Theme.of(context).textTheme.titleSmall),
                                            // TODO: Listar sessões do treinamento aqui
                                            const Text('Sessões serão listadas aqui.'), // Placeholder
                                            // Exemplo de como adicionar observações para sessões ativas
                                            if (treinamento.status == 'ativo') // Assumindo que 'ativo' é um status de treinamento
                                              Padding(
                                                padding: const EdgeInsets.only(top: 10.0),
                                                child: TextFormField(
                                                  decoration: const InputDecoration(
                                                    labelText: 'Adicionar Observações para a Sessão Ativa',
